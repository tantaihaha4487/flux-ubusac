...
The sidebar bundles Kanit locally in `fonts/`, so the panel uses it even when the system font is missing. The UI leans on lighter Kanit weights for a softer dashboard feel, and the header title uses a lighter weight for readability.
