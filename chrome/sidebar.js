(function () {
  ...
  function loadAll() {
    return storageGet(settings.configKeys.concat(settings.statusKey)).then(function (stored) {
  ...
  function saveConfig(config, message) {
    fillForm(config);
    return storageSet(config).then(function () {
  ...
  document.getElementById("export-settings").addEventListener("click", function () {
    storageGet(settings.configKeys).then(function (stored) {
